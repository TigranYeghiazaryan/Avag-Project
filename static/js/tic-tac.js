...
var game = {
  ...
  $identityBtn.on('click', function() {
    ...
    if (game.playerMark === "X") {
        game.aiMark = "O";
        game.playerTurn = true; // Игрок, выбравший X, начинает первым
    } else {
        game.aiMark = "X";
        game.playerTurn = false; // Игрок, выбравший O, начинает вторым
    }
  ...
  */
  ...
  function gameOver(winCase) {
    ...
    if (winCase === game.playerMark) {
      ...
      // PC wins
      $("#game-end-heading").text("Ավաղ, համակարգիչը հաղթեց:)");
      $("#game-end-subheading").text("Թող նրանց հետքերը թաղվեն տիկ-տակի փառքի մեջ...");
    } else {
      // Draw
